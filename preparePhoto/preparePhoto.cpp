...
string glasses_cascade_name = "/usr/local/share/OpenCV/haarcascades/haarcascade_eye_tree_eyeglasses.xml";
...
//////////////////////////////////////////////
Mat rotate(Mat& image, double angle, Point2i centre)
{
	...
	while ((ent=readdir(rep)) != NULL)
	{
	 	...
	 	if ((imageName[nLen-1]=='g')&&(imageName[nLen-2]=='p')&&(imageName[nLen-3]=='j'))
	 	{
			...
			trace("lecture : "+string(imageName));
		    	imframe = imread(imageName,1);
			Mat frame;
		    	frame = imframe.clone(); // We do this to avoid clobbering ourself on the next pass
...
int detectAndDisplay( Mat frame )
{
	...
	{
		// tests with glasses
		glasses_cascade.detectMultiScale( faceROI, eyes, 1.1, 2, 0 |CASCADE_SCALE_IMAGE, Size(20, 20) );
